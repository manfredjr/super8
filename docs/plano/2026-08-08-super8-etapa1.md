...
- **Ponto de entrada chama `dbSeguro()`, nunca `db()` direto.** `db()` constroi a conexao na hora da chamada, entao banco fora do ar estoura excecao antes de qualquer try, e com `display_errors` ligado a mensagem do driver vai para a tela. `dbSeguro()` registra o erro real no log e mostra mensagem generica. Duas excecoes de proposito: teste usa `db()`, porque `dbSeguro()` chama `exit` e mataria a suite; e `admin/anonimizar.php` usa `db()`, porque roda por linha de comando e nao responde com codigo HTTP. As telas de termo de uso e de politica de privacidade nao chamam nenhuma das duas, porque nao tocam o banco: sao exigencia legal e nao tem por que depender de ele estar no ar.
...
- **Marca da MT - Manfred Tecnologia** presente em toda tela que o competidor ve: pagina do evento, chaveamento, classificacao e ranking. Ela e requisito do modelo de negocio, nao enfeite: o organizador usa de graca e em troca a MT figura como apoiadora do campeonato. Com um limite firme: a tela de placar e a de chaveamento sao usadas no celular na beira da quadra, entre partidas, e marca que rouba espaco da informacao do jogo estraga o produto e a publicidade ao mesmo tempo. Presente e identificavel, nunca competindo com o placar.
- **Nunca citar a ferramenta de desenvolvimento** em codigo, comentario, tela, documento, nome de arquivo, nome de pasta ou mensagem de commit. Nao ha autorizacao previa para isso, e a autorizacao so pode vir do Manfred, por escrito, respondendo a uma pergunta especifica. Mencao feita sem autorizacao e apagada e o trabalho refeito.
...
| `src/Auth.php` | cadastro, login, bloqueio por tentativa, registro de aceite do termo |
...
| `views/marca.php` | a marca da MT, unico lugar que a desenha |
| `views/termo.php` `views/privacidade.php` | termo de uso e politica de privacidade |
| `views/*.php` | telas |
| `public/*.php` | pontos de entrada |
| `sql/schema.sql` | criacao das tabelas |
| `testes/*.php` | testes por linha de comando |
| `admin/anonimizar.php` | exclusao a pedido do titular, por linha de comando (tarefa 16) |
...
## Tarefa 10: Layout, marca da MT, termo de uso, login e cadastro

Esta e a porta de entrada do sistema, e ela carrega tres coisas que so existem juntas: o layout que todas as telas seguintes usam, a marca da MT que sustenta o modelo de negocio, e o aceite do termo que sustenta a base legal do modelo.

**Arquivos:**
- Criar: `public/cabecalho.php`
- Criar: `views/layout.php`, `views/marca.php`, `views/login.php`, `views/termo.php`, `views/privacidade.php`
- Criar: `public/css/estilo.css`
- Criar: `public/login.php`, `public/logout.php`, `public/termo.php`, `public/privacidade.php`
- Modificar: `config/config.exemplo.php` e `config/config.php`, para a versao do termo
- Modificar: `config/acesso.php`, para a exigencia de termo aceito

**Interfaces:**
- Consome: `Auth::cadastrar`, `Auth::autenticar`, `Auth::registrarAceite`, `Auth::versaoAceita`, `Auth::bloqueadoAte`, `Auth::registrarFalha`, `Auth::limparFalhas` (tarefa 6); `iniciarSessao()` e `csrf_*` (tarefa 5); `db()` (tarefa 4).
- Produz: `public/cabecalho.php`, que todo ponto de entrada inclui na primeira linha; `views/marca.php`, o unico lugar do sistema que desenha a marca da MT; `exigirTermoAceito()` em `config/acesso.php`.

**Cuidados que vem de fora desta tarefa:**

`Auth::cadastrar` lanca `InvalidArgumentException` para valor que o formulario podia ter conferido antes, e `RuntimeException` para o estado guardado recusando, como e-mail ja cadastrado. O controlador precisa capturar os dois.

`Auth::cadastrar` nao gerencia transacao. A conta e o aceite do termo tem que comitar juntos, ou existe conta real sem a base legal que o modelo de negocio exige. Quem abre a transacao e o controlador.

- [ ] **Passo 1: a versao do termo em vigor**

Em `config/config.exemplo.php` e em `config/config.php`, acrescentar:
...
- [ ] **Passo 2: escrever `public/cabecalho.php`**
...
- [ ] **Passo 3: escrever `views/marca.php`**

Este e o unico lugar do sistema que desenha a marca da MT. Toda tela que precisa dela inclui este arquivo. Trocar texto por imagem depois mexe aqui e em mais nenhum lugar.
...
- [ ] **Passo 4: escrever `views/layout.php`**
...
- [ ] **Passo 5: escrever `public/css/estilo.css`**

```css
:root { --azul: #1d4ed8; --cinza: #f4f4f5; --borda: #d4d4d8; --texto: #18181b;
        --apoio-fundo: #e6f1fb; --apoio-texto: #0c447c; }
...
main { flex: 1; width: 100%; max-width: 900px; margin: 0 auto; padding: 1rem; }
h1 { font-size: 1.4rem; }
form { display: grid; gap: .75rem; max-width: 460px; }
label { display: grid; gap: .25rem; font-size: .9rem; }
label.linha { display: flex; align-items: flex-start; gap: .5rem; font-size: .9rem; }
label.linha input { width: auto; margin-top: .2rem; }
...
.documento h2 { font-size: 1.1rem; margin-top: 1.5rem; }
.rodape { border-top: 1px solid var(--borda); padding: 1rem; }
.rodape-links { margin: .5rem 0 0; font-size: .8rem; }
.rodape-links a { color: var(--texto); margin-right: 1rem; }
.marca-discreta { margin: 0; font-size: .8rem; color: #52525b; }
.marca-apoio { display: flex; flex-direction: column; gap: .1rem; padding: .6rem .8rem;
               background: var(--apoio-fundo); border-radius: 6px; }
.marca-rotulo { font-size: .7rem; text-transform: uppercase; letter-spacing: .04em; color: #185fa5; }
.marca-nome { font-size: .95rem; color: var(--apoio-texto); }
@media (max-width: 600px) { main { padding: .75rem; } .placar input { width: 3.4rem; } }
```

- [ ] **Passo 6: escrever `views/termo.php`**

O texto abaixo e um primeiro rascunho e nao esta aprovado. Ele entra no sistema para a tela existir e o fluxo funcionar, e precisa da leitura do Manfred antes de o sistema receber organizador de fora. A especificacao registra que tambem merece leitura de advogado.

```php
<?php /** @var string $versao */ ?>
<div class="documento">
<p>Versao <?= e($versao) ?></p>

<h2>1. O que e este servico</h2>
<p>O Super 8 e um sistema para criar e gerenciar torneios de padel no formato Super 8, oferecido pela MT - Manfred Tecnologia.</p>

<h2>2. O servico e gratuito, e por que</h2>
<p>Voce nao paga nada para usar o sistema, nem para criar campeonatos, nem por evento, nem por jogador. Em troca, a MT - Manfred Tecnologia figura como apoiadora e patrocinadora de cada campeonato que voce criar aqui, com a marca dela visivel nas telas do evento, do chaveamento, da classificacao e do ranking.</p>
<p>Ao criar sua conta voce concorda com isso. Nao ha como usar o sistema sem essa contrapartida, porque e ela que paga o servico.</p>

<h2>3. O que aparece nas telas</h2>
<p>O nome dos competidores que voce cadastrar aparece no chaveamento, na classificacao do evento e no ranking acumulado, junto da marca da MT. Os resultados das partidas tambem.</p>

<h2>4. Sua responsabilidade ao inscrever outras pessoas</h2>
<p>Voce pode cadastrar competidores informando apenas o nome, sem que eles tenham conta no sistema. Nesse caso eles nao aceitam nada, porque nao passam por cadastro.</p>
<p>Cabe a voce informar essas pessoas de que o nome delas vai aparecer nas telas do sistema, junto da marca da MT como apoiadora do evento. Ao aceitar este termo voce assume essa responsabilidade.</p>

<h2>5. Seus dados</h2>
<p>Guardamos seu nome e seu e-mail para identificar sua conta, e o endereco de origem no momento em que voce aceita este termo, como prova do aceite. Nao pedimos telefone, documento nem data de nascimento.</p>
<p>Nao vendemos dado, nao compartilhamos com terceiros e nao enviamos dado de jogador para servico de publicidade. O ganho da MT vem da marca dela aparecer, e nao de dado sair daqui.</p>
<p>A politica de privacidade detalha o tratamento.</p>

<h2>6. Encerrar sua conta</h2>
<p>Voce pode pedir a exclusao dos seus dados a qualquer momento. Quando isso acontece, seu nome e substituido por um identificador anonimo e sua conta e desativada. Os placares dos eventos permanecem, porque a partir dai sao numero de partida sem ligacao com pessoa identificavel.</p>

<h2>7. Limites do servico</h2>
<p>O sistema e oferecido como esta. Fazemos o possivel para que os calculos de chaveamento, classificacao e ranking estejam corretos, e eles sao conferidos automaticamente, mas o resultado oficial de um torneio e responsabilidade de quem organiza.</p>

<h2>8. Mudancas neste termo</h2>
<p>Se este termo mudar, voce vera a nova versao ao entrar e precisara aceita-la para continuar usando o sistema. As versoes que voce ja aceitou ficam registradas com data.</p>

<h2>9. Contato</h2>
<p>MT - Manfred Tecnologia.</p>
</div>
```

- [ ] **Passo 7: escrever `views/privacidade.php`**

Mesmo aviso do passo anterior: rascunho, pendente de aprovacao.

```php
<div class="documento">
<h2>Quem trata seus dados</h2>
<p>MT - Manfred Tecnologia, responsavel pelo sistema Super 8.</p>

<h2>O que guardamos</h2>
<p>De quem cria conta: nome, e-mail, senha guardada de forma cifrada e irreversivel, e o endereco de origem no momento do aceite do termo de uso.</p>
<p>De quem e cadastrado como competidor sem conta: apenas o nome de exibicao informado por quem organiza o evento.</p>
<p>Dos eventos: nome, data, local, custo, os competidores inscritos e os placares das partidas.</p>

<h2>Por que guardamos</h2>
<p>Para o organizador, os dados existem para executar o servico que ele contratou ao criar a conta, que e o uso gratuito do sistema tendo a MT como apoiadora dos eventos dele.</p>
<p>Para o competidor cadastrado por terceiro, os dados existem pelo interesse legitimo de organizar o torneio: o nome aparece porque a pessoa esta jogando. A marca da MT esta na mesma pagina como patrocinadora do evento, do mesmo jeito que um banner de patrocinador aparece atras da quadra num torneio presencial. Nao usamos nome nem imagem de jogador em peca publicitaria.</p>

<h2>Com quem compartilhamos</h2>
<p>Com ninguem. Nao vendemos dado, nao cedemos para terceiros e nao alimentamos servico de publicidade com dado de quem usa o sistema.</p>

<h2>Por quanto tempo</h2>
<p>Enquanto a conta estiver ativa. Encerrada a conta, o nome e substituido por identificador anonimo e os placares permanecem como numero de evento.</p>

<h2>Seus direitos</h2>
<p>Voce pode pedir acesso aos seus dados, correcao e exclusao. O pedido pode ser feito pelo contato da MT - Manfred Tecnologia.</p>
</div>
```

- [ ] **Passo 8: escrever `public/termo.php` e `public/privacidade.php`**
...
E o de privacidade, igual, trocando o titulo para `Politica de privacidade` e a view para `privacidade.php`, sem a variavel de versao.

As duas telas ficam abertas, sem exigir login. Alguem precisa poder ler o termo antes de aceitar.

- [ ] **Passo 9: escrever `views/login.php`**
...
O aceite e caixa que comeca desmarcada, e nunca pre-marcada. Aceite pre-marcado nao e aceite.

- [ ] **Passo 10: escrever `public/login.php`**
...
A mensagem de erro e a mesma para e-mail inexistente e para senha errada. Diferenciar as duas entregaria a lista de e-mails cadastrados.

- [ ] **Passo 11: escrever `public/logout.php`**
...
- [ ] **Passo 12: a exigencia de termo aceito, em `config/acesso.php`**
...
A tela de aceite da nova versao, `public/aceite.php`, fica para quando o termo mudar de versao pela primeira vez. Nesta etapa existe uma versao so, e todo cadastro ja nasce com ela aceita. Registrar aqui para a funcao nao ser escrita depois sem o motivo.

- [ ] **Passo 13: sincronizar e testar no navegador**

```
powershell -ExecutionPolicy Bypass -File ferramentas\sincronizar-htdocs.ps1
```

Abrir `http://localhost/super8/public/login.php` e conferir:

- Criar conta sem marcar o aceite e recusado.
- Criar conta com aceite funciona, entra direto, e a tabela `aceites_termo` ganhou uma linha com a versao, a data e o endereco.
- O termo e a politica abrem sem login, pelos links do rodape e pelos links do formulario.
- E-mail ja cadastrado da mensagem de e-mail ja cadastrado, e nao erro de servidor.
- Senha com menos de 8 caracteres e recusada com mensagem clara.
- Cinco tentativas de senha errada bloqueiam com mensagem de horario.
- A marca da MT aparece no rodape de todas as telas.
- Em largura de celular, o formulario e os links continuam usaveis.

Conferir tambem, no banco, que uma tentativa de cadastro que falha por e-mail repetido nao deixou linha em `users` nem em `aceites_termo`. E o que prova que a transacao esta fazendo o trabalho dela.

- [ ] **Passo 14: commit**

```bash
git add public/ views/ config/
git commit -m "feat: layout, marca da MT, termo de uso, login e cadastro"
```
...
// Tela de operacao, usada em pe na beira da quadra. A marca fica discreta aqui
// para nao roubar espaco do placar. Ver views/marca.php.
$marcaDiscreta = true;
...
// Quem confere pendencia e muda o status e Campeonato::encerrar, sob a trava da
// linha do campeonato. Repetir a conferencia aqui, fora da trava, nao acrescenta
// nada e abriria espaco para um placar entrar entre a conferencia e a escrita.
try {
...
Nada. A politica de privacidade passou a ser obrigatoria nesta etapa, nao na etapa 2, porque o modelo de negocio tem finalidade publicitaria e a pessoa precisa poder ler onde seu dado vai antes de aceitar. Entrou na tarefa 10.
```
